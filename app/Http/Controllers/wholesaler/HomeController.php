...
class HomeController extends Controller
{
     public function index(){
        ...
        return view('wholesaler.home.index')
            ->with(compact('user', 'current_fy_date', 'order', 'charts_data', 'target_tonnage', 'notifications'));
    }
     ...
     public function distributor_chart_data($distributor_id){
        ...
        $output_point = Point::where('user_id', $distributor_id)
                        ->select(
                        DB::raw("SUM(IF((created_at >= '$fy0[start]' AND created_at <= '$fy0[end]:23:59:00'), points, 0)) as fy0_reward"),
                        DB::raw("SUM(IF(created_at >= '$fy1[start]' AND created_at <= '$fy1[end]:23:59:00', points, 0)) as fy1_reward"),
                        DB::raw("SUM(IF(created_at >= '$fy2[start]' AND created_at <= '$fy2[end]:23:59:00', points, 0)) as fy2_reward"),
                        DB::raw("SUM(IF(created_at >= '$fy3[start]' AND created_at <= '$fy3[end]:23:59:00', points, 0)) as fy3_reward"),
                        DB::raw("SUM(IF(created_at >= '$fy4[start]' AND created_at <= '$fy4[end]:23:59:00', points, 0)) as fy4_reward"),
                    )
                    ->first();            
