...
namespace App\Http\Controllers\SuperStockist;
...
class HomeController extends Controller
{
    public function index()
    {
        ...
        return view('super_stockist.home.index')
    ...
    public function distributor_chart_data($distributor_id)
    {
        ...
        $output_point = Point::where('user_id', $distributor_id)
            ->select(
                ...
                DB::raw("SUM(CASE WHEN operation = 'add' AND created_at >= '$fy0[start]' AND created_at <= '$fy0[end]' then points
                            WHEN operation = 'sub' AND created_at >= '$fy0[start]' AND created_at <= '$fy0[end]' then -points END) as fy0_reward"),

                DB::raw("SUM(CASE WHEN operation = 'add' AND created_at >= '$fy1[start]' AND created_at <= '$fy1[end]' then points
                            WHEN operation = 'sub' AND created_at >= '$fy1[start]' AND created_at <= '$fy1[end]' then -points END) as fy1_reward"),

                DB::raw("SUM(CASE WHEN operation = 'add' AND created_at >= '$fy2[start]' AND created_at <= '$fy2[end]' then points
                            WHEN operation = 'sub' AND created_at >= '$fy2[start]' AND created_at <= '$fy2[end]' then -points END) as fy2_reward"),

                DB::raw("SUM(CASE WHEN operation = 'add' AND created_at >= '$fy3[start]' AND created_at <= '$fy3[end]' then points
                            WHEN operation = 'sub' AND created_at >= '$fy3[start]' AND created_at <= '$fy3[end]' then -points END) as fy3_reward"),

                DB::raw("SUM(CASE WHEN operation = 'add' AND created_at >= '$fy4[start]' AND created_at <= '$fy4[end]' then points
                            WHEN operation = 'sub' AND created_at >= '$fy4[start]' AND created_at <= '$fy4[end]' then -points END) as fy4_reward"),
