...
@section('content')
    ...
    <form method="POST" action="{{ route(prefix_route('orders.store')) }}" id="addorder">
        <div class="row g-5 g-xl-12">
            @csrf
            <div class="col-md-4">
                @include('orders.partials.add-product-column', [
                    'heading' => 'COW GHEE',
                    'pack_type' => 'tin',
                ...
                <div class="card-body">
                    <div class="rounded border d-flex flex-column p-10">
                        @if (Auth::user()->role == 'distributor' && number_format($rewards) > 0)
                            <div class="col-md-4">
                                <label for="rewards" class="form-label">Your credit rewards Rs .
                                    {{ number_format($rewards) }}</label>
                                <br>
                                <label for="rewards" class="form-label">Use credit Rewards</label>
                                <input class="form-control form-control-solid" name="rewards" id="rewards" type="number"
                                    step="any">
                            </div>
                            <br>
                        @endif
                        <label for="" class="form-label">Additional notes for order</label>
                        <textarea class="form-control form-control-solid" rows=5 name="distributor_notes"></textarea>
                    </div>
                </div>
