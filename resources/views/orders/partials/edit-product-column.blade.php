        ...
        <table class="table">
            ...
            @foreach ($products as $product)
                @if ($product->pack_type == $pack_type && $product->product_type == $product_type)
                    @if (($product->barcodes != 'CJ0012' || (auth()->user()->role != 'sub_stockist' && !isset($_GET['ordertype']))) && ($product->barcodes != 'CJ0005' || (auth()->user()->role == 'sub_stockist' || isset($_GET['ordertype']))))
                        @php
                            $quantity = isset($order_lines_formatted[$product->id]) ? $order_lines_formatted[$product->id]->quantity : '';
                            
                            $order_line_id = isset($order_lines_formatted[$product->id]) ? $order_lines_formatted[$product->id]->id : null;
                        @endphp
                            ...
                            <td>
                                <div class="col-md-12">
                                    <div class="input-group">
                                        ...
                                        <input type="number" min=0 class="form-control quantity" placeholder="quantity"
                                            name="products[{{ $product->id }}][quantity]" value="{{ $quantity }}">

                                        @if (!is_null($order_line_id))
                                            <input type="hidden" name="products[{{ $product->id }}][order_line_id]"
                                                value="{{ $order_line_id }}">
                                        @endif
